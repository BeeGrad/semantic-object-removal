...
class TrainOptions:
    ...
    def initialize(self):
        ...
        self.parser.add_argument(
            ...
            type=str,
            default="",
            help="the file storing training image paths",
        )
        self.parser.add_argument(
            "--gpu_ids", type=str, default="0", help="gpu ids: e.g. 0  0,1,2"
        )
        self.parser.add_argument(
            "--checkpoint_dir",
            type=str,
        ...
        self.parser.add_argument(
            ...
            type=str,
            default="",
            help="pretrained models are given here",
        )
        ...
        self.parser.add_argument(
            "--vgg19_path",
            type=str,
            default="vgg19_weights/imagenet-vgg-verydeep-19.mat",
        )
    ...
    def parse(self):
        ...
        self.opt.model_folder += (
            "_s" + str(self.opt.img_shapes[0]) + "x" + str(self.opt.img_shapes[1])
        )
        self.opt.model_folder += "_gc" + str(self.opt.g_cnum)
        self.opt.model_folder += "_dc" + str(self.opt.d_cnum)

        self.opt.model_folder += (
            "_randmask-" + self.opt.mask_type if self.opt.random_mask else ""
        )
        ...
        self.opt.model_folder = os.path.join(
            self.opt.checkpoint_dir, self.opt.model_folder
        )
...
class TestOptions:
    ...
    def initialize(self):
        ...
        self.parser.add_argument(
            "--data_file",
            type=str,
            default="./imgs/paris-streetview_256x256",
        ...
        self.parser.add_argument(
            "--random_mask", type=int, default=0, help="using random mask"
        )
    ...
    def parse(self):
        ...
        self.opt.model_folder = (
            self.opt.date_str + "_" + self.opt.dataset + "_" + self.opt.model
        )
        self.opt.model_folder += (
            "_s" + str(self.opt.img_shapes[0]) + "x" + str(self.opt.img_shapes[1])
        )
