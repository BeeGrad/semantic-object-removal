...
class DataRead:
    ...
    def get_data(self):
        ...
        if self.dataset == "cifar10":
            ...
            print(
                f"Data Array has been created from cifar10 dataset with shape: {data.shape}"
            )
            self.data = data
    ...
    def create_masked_data(self):
        ...
        if self.masking_type == "lines":
            for img in range(self.data.shape[0]):
                mask = np.full(
                    (image_width, image_heigth, image_channel), 255, np.uint8
                )  ## White background

                for _ in range(np.random.randint(1, image_width // 4)):
                    # Get random x locations to start line
                    x1, x2 = (
                        np.random.randint(1, image_width),
                        np.random.randint(1, image_width),
                    )
                    # Get random y locations to start line
                    y1, y2 = (
                        np.random.randint(1, image_heigth),
                        np.random.randint(1, image_heigth),
                    )
