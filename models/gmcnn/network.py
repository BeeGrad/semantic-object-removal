...
from models.gmcnn.basemodel import BaseModel
from models.gmcnn.basenet import BaseNet
from scripts.loss import WGANLoss, IDMRFLoss
from models.gmcnn.layer import init_weights, PureUpsampling, ConfidenceDrivenMaskLayer, SpectralNorm
...
class GMCNN(BaseNet):
    def __init__(
        ...
        self.decoding_layers.append(
            nn.Conv2d(ch // 2, out_channels, kernel_size=3, stride=1)
        )
...
class Discriminator(BaseNet):
    def __init__(
        ...
        if spectral_norm:
            self.layers.append(
                SpectralNorm(
                    nn.Conv2d(in_channels, ch, kernel_size=5, padding=2, stride=2)
                )
            )
            self.layers.append(
                SpectralNorm(nn.Conv2d(ch, ch * 2, kernel_size=5, padding=2, stride=2))
            )
            self.layers.append(
                SpectralNorm(
                    nn.Conv2d(ch * 2, ch * 4, kernel_size=5, padding=2, stride=2)
                )
            )
            self.layers.append(
                SpectralNorm(
                    nn.Conv2d(ch * 4, ch * 4, kernel_size=5, padding=2, stride=2)
                )
            )
            ...
            self.layers.append(
                nn.Conv2d(ch, ch * 2, kernel_size=5, padding=2, stride=2)
            )
            self.layers.append(
                nn.Conv2d(ch * 2, ch * 4, kernel_size=5, padding=2, stride=2)
            )
...
class InpaintingModel_GMCNN(BaseModel):
    def __init__(self, in_channels, act=F.elu, norm=None, opt=None):
        ...
        self.netGM = GMCNN(
            in_channels, out_channels=3, cnum=opt.g_cnum, act=act, norm=norm
        ).cuda()
        ...
        if self.opt.pretrain_network is False:
            if self.opt.mask_type == "rect":
                self.netD = GlobalLocalDiscriminator(
                    ...
                    cnum=opt.d_cnum,
                    act=act,
                    g_fc_channels=opt.img_shapes[0]
                    // 16
                    * opt.img_shapes[1]
                    // 16
                    * opt.d_cnum
                    * 4,
                    l_fc_channels=opt.mask_shapes[0]
                    // 16
                    * opt.mask_shapes[1]
                    // 16
                    * opt.d_cnum
                ...
                self.netD = GlobalLocalDiscriminator(
                    ...
                    * opt.d_cnum
                    * 4,
                    l_fc_channels=opt.img_shapes[0]
                    // 16
                    * opt.img_shapes[1]
                    // 16
                    * opt.d_cnum
    ...
    def initVariables(self):
        ...
        mask, rect = generate_mask(
            self.opt.mask_type, self.opt.img_shapes, self.opt.mask_shapes
        )
        self.mask_01 = (
            torch.from_numpy(mask).cuda().repeat([self.opt.batch_size, 1, 1, 1])
        )
    ...
    def forward_G(self):
        ...
        if self.opt.pretrain_network is False:
            ...
            self.G_loss_mrf = self.mrfloss(
                (self.completed_local + 1) / 2.0, (self.gt_local.detach() + 1) / 2.0
            )
