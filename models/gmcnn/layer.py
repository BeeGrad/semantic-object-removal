...
class upsampling(nn.Module):
    ...
    def forward(self, x):
        ...
        xout = self.conv(
            F.interpolate(input=x, size=(h, w), mode="nearest", align_corners=True)
        )
        return xout
...
class PureUpsampling(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mode == "nearest":
            ...
            xout = F.interpolate(
                input=x, size=(h, w), mode=self.mode, align_corners=True
            )
        return xout
...
class PartialConv(nn.Module):
    def __init__(self, in_channels=3, out_channels=32, ksize=3, stride=1):
        ...
        self.eplison = 1e-5
        self.conv = nn.Conv2d(
            in_channels, out_channels, stride=stride, kernel_size=ksize
        )

    def forward(self, x, mask):

        mask_ch = mask.size(1)
        sum_kernel_np = np.ones(
            (mask_ch, mask_ch, self.ksize, self.ksize), dtype=np.float32
        )
        ...
        x = (
            ...
            * mask
            / (
                F.conv2d(mask, sum_kernel, stride=1, padding=self.padSize)
                + self.eplison
            )
        )
        x = self.pad(x)
...
class GatedDilatedConv(nn.Module):
    def __init__(
        self, in_channels, out_channels, ksize=3, stride=1, pad=1, dilation=2, act=F.elu
    ):
