...
class FPN(nn.Module):
    def __init__(self, block=Bottleneck, num_blocks=2):
    ...
    def forward(self, x):
        ...
        p3 = self.smooth2(p3)
        p2 = self.smooth3(p2)
        # print(f"FPN 0 Output: {p2.detach().numpy().shape}")
        # print(f"FPN 1 Output: {p3.detach().numpy().shape}")
        # print(f"FPN 2 Output: {p4.detach().numpy().shape}")
        # print(f"FPN 3 Output: {p5.detach().numpy().shape}")
        return p2, p3, p4, p5
...
class InpaintingModel(nn.Module):
    ...
    def step(self, features, masks, images):
        # process outputs
